...
| Charging Core (Station/EVSE/Connector) | Partial          |    40% | Yes (CRUD only)      | Not connected (still hardcoded demo) | Yes (CAP-002)         | Yes (CRUD only)       | NO (no OCPP, no sessions) |
...
Sessions, Tariffs, Alerts, Reporting, Billing, and Notifications all ultimately depend on the Charging Core (Station/EVSE/Connector) existing, which now has a CRUD-only backend as of CAP-002. None of those dependents are unblocked yet — they need OCPP communication and/or session/tariff models that CAP-002 explicitly did not implement. See the [Dependency Map](./MOVOS_DEPENDENCY_MAP_v1.0.md) for the full graph.
