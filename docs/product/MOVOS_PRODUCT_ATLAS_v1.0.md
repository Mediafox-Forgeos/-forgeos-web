...
- **Charging Sessions & Authorization (CAP-004 + WO-ARGOS-009)** — `apps/movos-api/src/{sessions,authorization}/` (real `ChargingSession`/`AuthorizationCredential`/`AuthorizationAttempt`/`MeterValue` models, validated session-lifecycle state machine, `Authorize`/`StartTransaction`/`MeterValues`/`StopTransaction` handled for 1.6J, read + credential-write APIs); no billing/tariffs/payments/reservations, no RFID-specific logic, no `Driver`/`Vehicle`/`Fleet` models, no live-database validation performed as of this work order — see [Charging Session Guide](../engineering/CHARGING_SESSION_GUIDE.md)
...
Tariffs · Alerts · Reporting · Users (team management — the underlying `User`/`Membership` models exist, but no API exposes them for this purpose) · Notifications · `packages/ui` (scaffolded, zero consumers) · Billing (no artifact at all) · Vehicles / Fleet (no artifact at all — these terms never entered this codebase's vocabulary; see Domain Inventory)
