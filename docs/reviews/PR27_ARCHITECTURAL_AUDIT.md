...
## ARGOS Risk Acceptance Record (2026-08-02, WO-ARGOS-011A)

**Decision:** "ARGOS authorizes merge with documented risk acceptance."

All 13 findings in this document and in [PR27_RISK_MATRIX.md](./PR27_RISK_MATRIX.md) (R-01 through R-13) — including the most severe, R-01 (the concurrent-reconnect-vs-StartTransaction connector race that can produce two live `ChargingSession` rows), R-03 (the clean-disconnect orphan-session gap), and R-11 (`sweepStale()`'s unthrottled mass-disconnect fan-out) — are accepted as known, documented risk at current MVP/single-instance scale. None are fixed by this record; none are silently dropped either — they remain open, numbered findings, not resolved ones. This record changes their disposition from "blocking an unconditional merge" to "accepted and merged with the risk on record," nothing more.

This acceptance is scoped to the state of PR #27 at HEAD `2a895cb89a81fe37dcb4c08ba5523485a4727e9d`, the commit merged as a result of this decision. It does not pre-approve any future change to `ConnectivityCoordinator`, `ConnectionRegistryService`, or `SessionLifecycleService` — a future capability that meaningfully changes the concurrency profile of these components (e.g., multi-instance routing, higher reconnect rates) should re-examine whether these findings still hold at the same severity, not assume this record still applies unchanged.
