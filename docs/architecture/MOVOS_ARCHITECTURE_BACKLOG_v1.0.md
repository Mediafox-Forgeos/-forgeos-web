...
**Updated:** 2026-08-04 — WO-ARGOS-017/017A (CAP-009, PR #34 merged at `7bd032ea5c7168d1a6e3e816f40fab398ba7d90f`, tagged `CAP-009_FOUNDATION_COMPLETE`): entries #24 (Tariffs) and #25 (Billing) gain a data-model-status update — `TariffSnapshot`/`BillingAccount` are now real, database-enforced Prisma schema, not just approved architecture; entry #52 (CAP-009) itself moves from registered/not-started to **IMPLEMENTED** (foundation only — services remain interface-only, no API, no UI). New entry #53 registers CAP-010 (Invoice & Ledger Architecture) as the next capability, documentation-only scope.
...
**2026-08-04 update (WO-ARGOS-017/017A):** CAP-009 shipped `TariffSnapshot` as real schema (5 migrations, PR #34 merged at `7bd032ea5c7168d1a6e3e816f40fab398ba7d90f`) — `Decimal(12,6)`/`Decimal(12,2)` money fields (never `Float`), immutable/append-only by construction, cross-snapshot currency consistency enforced by a database trigger. Data-model status moves from None to **Schema complete**. The snapshot-triggering rule and sparse-telemetry energy-attribution rule named as open above remain open — CAP-009 built the container, not the pricing logic that decides when to fill it.
...
**2026-08-04 update (WO-ARGOS-017/017A):** CAP-009 shipped `BillingAccount` as real schema (PR #34 merged at `7bd032ea5c7168d1a6e3e816f40fab398ba7d90f`, tagged `CAP-009_FOUNDATION_COMPLETE`) — five original account types plus a `SYSTEM_DEFAULT` type added during hardening (one per organization, auto-resolved for every session without an explicit debtor), a required composite tenant-isolation foreign key from `ChargingSession`, and `RESTRICT`-on-delete archival semantics. Data-model status moves from None to **Schema complete, service unimplemented**. No `Invoice` model, controller, or UI exists — `BillingAccountService` is an interface only. See #53 for the newly registered next capability (CAP-010 — Invoice & Ledger Architecture, documentation-only scope).
...
**2026-08-04 update (WO-ARGOS-017/017A):** **IMPLEMENTED.** PR #34 merged to `main` at `7bd032ea5c7168d1a6e3e816f40fab398ba7d90f`, tagged `CAP-009_FOUNDATION_COMPLETE`. `BillingAccount` and `TariffSnapshot` are real Prisma models (5 migrations); `ChargingSession.billingAccountId` is a required, tenant-isolation-enforcing composite foreign key (`RESTRICT` on delete, correcting Prisma's originally-shipped `SET NULL` default); cross-`TariffSnapshot` currency consistency is enforced by this schema's first database trigger. `BillingAccountService`/`TariffSnapshotService` ship as interfaces only — no implementing class, no controller, no route, no UI. Data-model status moves from None to **Schema complete, service unimplemented**; implementation status moves from None to **Foundation only**. Decisions still open above (snapshot-triggering rule, sparse-telemetry energy attribution, pricing clock, ephemeral-vs-durable `BillingAccount` shape) remain exactly as open — this entry closed structural/invariant questions, not pricing-calculation ones. See [CAP-009 Post-Mortem](../postmortems/CAP-009_POST_MORTEM.md), [CAP-009_BILLING_ACCOUNT_MODEL.md](../domain/CAP-009_BILLING_ACCOUNT_MODEL.md), [CAP-009_TARIFF_SNAPSHOT_MODEL.md](../domain/CAP-009_TARIFF_SNAPSHOT_MODEL.md), [CAP-009_INVARIANTS.md](../domain/CAP-009_INVARIANTS.md), [CAP-009_ARCHIVAL_POLICY.md](../domain/CAP-009_ARCHIVAL_POLICY.md). New entry #53 registers CAP-010 (Invoice & Ledger Architecture) as the next capability, documentation-only scope.

### 53. CAP-010: Invoice & Ledger Architecture

- **Business purpose:** design (documentation only, no implementation) how a priced, completed `ChargingSession` — now attributable to a `BillingAccount` via CAP-009's real schema — becomes an `Invoice` and a ledger entry: what an invoice line item is, how partial/aggregated billing (e.g., monthly statements across many sessions for one `BillingAccount`) works, how a failed or disputed payment is represented, and how the ledger stays reconcilable against `TariffSnapshot`-priced sessions.
- **Domain boundary:** Commercial — the architectural half of entries #26 (Payments) and #27 (Refunds), and the next layer above #25 (Billing)/#52 (CAP-009).
- **Related entities:** future `Invoice`, `LedgerEntry`/`InvoiceLineItem` (naming not yet decided); references existing `BillingAccount`, `TariffSnapshot`, `ChargingSession`.
- **Related protocols:** none.
- **Architectural status:** UNDEFINED — registered, not started.
- **Data-model status:** None.
- **Interface-contract status:** None.
- **Implementation status:** None.
- **Dependencies:** #52 CAP-009 (done — `BillingAccount`/`TariffSnapshot` foundation), #25 Billing, #24 Tariffs.
- **Decisions already approved:** none yet — this entry inherits CAP-008's canonical debt owner (`BillingAccount`) and CAP-009's tariff-timing mechanism (`TariffSnapshot`) as given inputs, not open questions.
- **Decisions still open:** invoice line-item shape and its relationship to one-or-many `TariffSnapshot`/`ChargingSession` rows; aggregation/statement period semantics; how a disputed or failed payment is represented in the ledger without touching Stripe/payment-processor integration itself; duplicate-invoice prevention's exact constraint shape (carried forward from entry #25); immutability/append-only requirements for the ledger, by analogy to `TariffSnapshot`'s own append-only design.
- **MVP relevance:** Not required for the MVP.
- **Recommended implementation phase:** Next capability — **documentation only, per explicit authorization**. Do not implement `Invoice`, `Payment`, `Stripe`, taxes, discounts, or accounting integration; no schema, no migration, no UI. Do not begin RFID (#51's remaining half), Smart Charging (#20), or OCPP 2.0.1 (#2) as part of this entry.
- **Risks if ignored:** None immediate — a priced, attributable `ChargingSession` exists (as of CAP-009) but nothing yet turns it into something a `BillingAccount` can be asked to pay.
- **Evidence source:** [CAP-009 Post-Mortem](../postmortems/CAP-009_POST_MORTEM.md), [CAP-008_BILLING_MODEL.md](../domain/CAP-008_BILLING_MODEL.md), [CAP-008_DEBT_OWNERSHIP.md](../domain/CAP-008_DEBT_OWNERSHIP.md).
