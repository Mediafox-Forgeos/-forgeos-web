...
## Addendum (2026-08-19, WO-ARGOS-063) — an expired OFFLINE session terminalized by a new transaction, not by connectivity loss itself

`EXPIRED_OFFLINE_SESSION_CANNOT_ABSORB_NEW_TRANSACTION`: `SessionLifecycleService.createSession` now transitions an `OFFLINE` session outside `isOfflineSessionRecoverable`'s window to `FAILED` when a genuinely new `StartTransaction` arrives on the same connector, rather than silently reusing that row (WO-ARGOS-062's discovered collision).

**This does not weaken item 5.** The trigger for this `FAILED` transition is never the disconnect itself — it is the arrival of a new, protocol-driven `StartTransaction`, which is exactly the kind of explicit protocol event item 5 already carves out as legitimate ("Terminal transitions remain exclusively protocol-driven... or explicit administrative action"). Connectivity loss alone, with no follow-up transaction, still leaves a session `OFFLINE` indefinitely (or until an operator acts) — item 5's absolute rule is unchanged. What's new is only that MOVOS now recognizes a specific protocol event (a new `StartTransaction` on an already-`OFFLINE`-past-window connector) as sufficient evidence that the old transaction is abandoned, and terminalizes it in the same breath as accepting the new one.
