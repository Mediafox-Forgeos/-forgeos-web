...
**For the precise, line-level mechanics of the grace window** (exactly when it starts, behavior at 2/3/5 concurrent tabs, replay-outside-the-window guarantees, whether rotation-on-use is preserved, the exact bound on what security guarantee is traded, and the literal request timeline) **see [DEC-022_REFRESH_RACE_CLARIFICATION.md](./DEC-022_REFRESH_RACE_CLARIFICATION.md)**, written in response to ARGOS's review questions on PR #31. This section remains the design-level summary; that document is the exhaustive answer.
