...
**WO-ARGOS-063 addendum.** Check 3's window arithmetic now lives in exactly one place — `SessionLifecycleService.isOfflineSessionRecoverable` (exported as `OFFLINE_RECOVERY_WINDOW_MS`) — and `attemptRecovery` calls it rather than computing it inline; `ConnectivityCoordinator.RECOVERY_WINDOW_MS` is kept as a re-exported alias of the same constant for backward compatibility. The same method now also gates `SessionLifecycleService.createSession`'s connector-occupancy check: an `OFFLINE` session outside this window is no longer just "unrecoverable on reconnect" — a genuinely new `StartTransaction` on that connector explicitly terminalizes it to `FAILED` (`EXPIRED_OFFLINE_SESSION_CANNOT_ABSORB_NEW_TRANSACTION`, see CAP-004 §13) rather than silently reusing it. Recovery's own behavior above (reconnect-triggered) is unchanged by this — the two paths share only the recoverability definition, not any new coupling.
...
RFID-specific behavior · billing/tariffs/invoices/payments · remote start/stop · OCPP 2.0.1 functional messages · smart charging/vendor profiles · Redis or any message broker · high-availability multi-instance connection routing · SLA analytics/alerting workflows · per-station-configurable heartbeat intervals (still a single hardcoded global 300s, per DEC-017's own noted limitation). The clean-disconnect-to-OFFLINE gap formerly noted here was resolved by WO-ARGOS-061 (see §4). The `OFFLINE`-session/new-`StartTransaction` collision formerly noted here (`MOVOS_OFFLINE_SESSION_NEW_STARTTRANSACTION_COLLISION`) was resolved by WO-ARGOS-063 (see §6's addendum and CAP-004 §13) — `SessionLifecycleService.createSession`'s occupancy guard now excludes an out-of-window `OFFLINE` session rather than absorbing a genuinely new transaction into it. Still tracked as a separate, genuinely open follow-up, not this capability: `MOVOS_SUSPENDED_SESSION_REACHABILITY` (`ChargingSessionStatus.SUSPENDED` is modeled but unreachable from any real production message flow today).
