import type { EngineInput, FilterResult } from './types.js';
import {
...
function filterDictionaryWords(name: string): FilterResult {
  ...
  if (COMMON_ENGLISH_WORDS.has(lower))
    return { status: 'fail', reason: 'common English word' };
  if (COMMON_SPANISH_WORDS.has(lower))
    return { status: 'fail', reason: 'common Spanish word' };
  return { status: 'pass' };
}
...
function filterConsecutiveConsonants(name: string): FilterResult {
  if (/[^AEIOU]{4,}/i.test(name))
    return { status: 'fail', reason: '4+ consecutive consonants' };
  return { status: 'pass' };
}

function filterExcessiveVowels(name: string): FilterResult {
  if (/[AEIOU]{3,}/i.test(name))
    return { status: 'fail', reason: '3+ consecutive vowels' };
  return { status: 'pass' };
}

// A name that is ALL consonants is unpronounceable
function filterAllConsonants(name: string): FilterResult {
  const hasVowel = /[AEIOU]/i.test(name);
  if (!hasVowel) return { status: 'fail', reason: 'no vowels' };
  return { status: 'pass' };
}
...
export function filterCandidates(
  ...
  for (const name of names) {
    ...
    if (result.status === 'pass') {
      ...
      rejected.push({ name, reason: result.reason ?? 'unknown' });
    }
  }
