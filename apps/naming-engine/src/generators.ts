import type { Strategy, EngineInput } from './types.js';
import {
  ONSETS_PREMIUM,
  VOWEL_NUCLEI,
  CODAS_STRONG,
...
export function generateInvented(input: EngineInput): string[] {
  ...
  // 3-syllable short forms: onset + vowel + onset + vowel + onset + vowel
...
// ─── Strategy B — Modified Latin roots ───────────────────────────────────────
...
// ─── Strategy C — Modified Greek roots ───────────────────────────────────────
...
export function generateGreek(input: EngineInput): string[] {
  ...
  for (const { root } of GREEK_ROOTS) {
    for (const suffix of GREEK_SUFFIXES) {
      ...
      // Drop last vowel then add suffix
      const stripped = root.replace(/[aeiou]$/i, '');
      if (stripped !== root) {
...
// ─── Strategy D — Phonetic construction ──────────────────────────────────────
  ...
  for (const cluster of CLUSTERS) {
    for (const bridge of BRIDGES) {
      for (const closer of CLOSERS) {
        ...
        // Insert a strong consonant before closer
...
// ─── Strategy E — Meaning-first generation ───────────────────────────────────

export function generateMeaning(input: EngineInput): string[] {
  const { min, max } = input.lengthConstraints;
  const results: string[] = [];

  // Derive fragments from keywords
...
// ─── Strategy F — Technology-style brands ────────────────────────────────────
...
// ─── Strategy G — Minimal brands (4–5 letters) ───────────────────────────────
...
export function generateMinimal(input: EngineInput): string[] {
  ...
  // Pattern: consonant pair + vowel + strong coda
